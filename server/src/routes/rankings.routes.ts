...
rankingsRouter.post('/generate/:jobId', auth, (req, res) => {
  ...
  try {
    ...
    for (const candidate of ranked) {
      io.to(`job:${jobId}`).emit('candidate_scored', { jobId, candidate });
    }
    io.to(`job:${jobId}`).emit('ranking_updated', { jobId, candidates: ranked });
  } catch {
...
rankingsRouter.get('/:jobId', auth, (req, res) => {
  ...
  res.json({
    ...
    candidates: candidates
      .filter((candidate) => candidate.jobId === req.params.jobId)
      .sort((a, b) => b.aiScore - a.aiScore)
  });
});
